...
from generator import create_generator, generator_loss
from discriminator import create_discriminator, discriminator_loss
    ...
    for epoch in range(config.adversarial_epochs):
        ...
        if config.training_loop == 'simultaneous':
            for i, image_batch in enumerate(dataset):
                ...
                wandb.log({'disc_grad': wandb.Histogram(np.array(metrics['disc_gradient'][0].values, dtype=float)),
                            'gen_grad': wandb.Histogram(np.array(metrics['gen_gradient'][0].values, dtype=float))})
